...
\subsubsection{Conjunción $\land$}
La conjunción de dos proposiciones $p$ y $q$ se denota como $p \land q$. Representa la idea de que ambas proposiciones son verdaderas. Por ejemplo, si $p$ es ``es lunes'' y $q$ es ``tengo una reunión'', entonces $p \land q$ sería ``es lunes y tengo una reunión''. Su tabla de verdad es:
...
\subsubsection{Doble implicación o bicondicional $\iff$}
...
\begin{enumerate}[label=\roman*)]
	\item \textit{Directo}: Si $p$ es $F$, nada hay que probar, pues en este caso $p \implies q$ es $V$. Si $p$ es $V$ hay que establecer que el valor de verdad de $q$ es $V$.
...
\underline{Nota}: También se tiene el \textit{método de inducción} del que se hablará en la sección \ref{sec:induccion}, pág. \pageref{sec:induccion}.
...
\begin{fmd-example}[Negación de una implicación \(\neg (p \implies q) \iff (p \land \neg q)  \)]
	...
	\begin{enumerate}[label=\roman*)]
		\item Si hoy es lunes, entonces mañana es miércoles.
		
		Cuya negación es: ``Hoy es lunes y mañana no es miércoles''
		
		\item ``Si estudias para el examen, entonces obtendrás una buena calificación''
		
		La negación de esta afirmación sería: ``Estudias para el examen, pero no obtienes una buena calificación''
		
		\[ p \implies q  \]
		
		Hemos negado la implicación original al afirmar que estudias pero no obtienes un buena calificación.
		
		\item ``Si un número es divisible por 4, entonces también es divisible por 2''
		
		La negación es: ``Hay un número que es divisible por 4, pero no es divisible por 2''
	\end{enumerate}
\end{fmd-example}
...
El \textit{razonamiento deductivo válido} es un proceso lógico en el que, a partir de premisas o afirmaciones, se llega a una conclusión de manera irrefutable. Un razonamiento es válido si la conclusión sigue necesariamente de las premisas, siguiendo las reglas de la lógica.
...
\begin{fmd-example}[Argumento válido]
	\[ \frac{\begin{array}{l}
			p \implies  q\\
			p
	\end{array}}{q} \]
...
\begin{fmd-example}[Argumento inválido]
	\[ \frac{\begin{array}{l}
			p \implies  q\\
			q
	\end{array}}{p} \]
...
\subsubsection{Afirmación del consecuente}
...
\subsubsection{Negación del antecedente}
\begin{itemize}
	\item Estructura:
		\[ \begin{array}{l}
		p \implies q\\
...
\subsubsection{Falacia del término medio no distribuido}
\begin{itemize}
	\item Estructura:
	\begin{itemize}[itemsep=-5pt]
...
\subsubsection{Falacia de la división}
\begin{itemize}
	\item Estructura:
	\begin{itemize}[itemsep=-5pt]
...
\begin{fmd-definition}[Función proposicional $P(x)$]
...
\section{Caracterización axiomática de los números naturales}
...
\begin{fmd-theorem}[Principio del buen orden]
...
\begin{fmd-theorem}[Teorema de Cantor]
