...
A continuación presentamos varios \glspl{operadorlogico} y sus correspondientes \gls{tablav}. En una tabla de verdad se evalúan todos los posibles valores de verdad de las proposiciones componentes y se determina el valor de verdad resultante de la proposición completa. \index{tabla!de verdad}
...
La \gls{conjuncion} de dos proposiciones $p$ y $q$ se denota como $p \land q$. Representa la idea de que ambas proposiciones son verdaderas. Por ejemplo, si $p$ es ``es lunes'' y $q$ es ``tengo una reunión'', entonces $p \land q$ sería ``es lunes y tengo una reunión''. Su tabla de verdad es:
...
La operación lógica de conjunción es análoga a la intersección de la teoría de conjuntos. La intersección $A \cap B$ contiene solo los elementos que pertenecen a $A$ y a $B$ ($p \land q$ es $V$).
...
La \gls{disyuncion} de dos proposiciones $p$ y $q$ se denota como $p \lor q$. Representa la idea de que al menos una de las proposiciones es verdadera. Por ejemplo, si $p$ es ``estudiaré'' y $q$ es ``veré una película'', entonces $p \lor q$ sería ``estudiaré o veré una película''. La tabla de verdad correspondiente es:
...
La operación lógica de disyunción es análoga a la unión de la teoría de conjuntos. La unión $A \cup B$ contiene los elementos que pertenecen a $A$ o a $B$ o a ambos, en otras palabras, siempre y cuando un elemento pertenezca a $A$ o a $B$ o a ambos $p \lor q$ será $V$.
...
Esta operación es análoga a la diferencia simétrica de la teoría de conjuntos. $A \triangle B$ contiene los elementos que pertenecen a $A$ o a $B$ pero no a ambos.
...
\subsubsection{Doble implicación o bicondicional $\iff$} \index{doble implicación} \index{implicación!doble} \index{implicación!condicional} \index{bicondicional} \index{si y solo si}
 La doble implicación, o \gls{bicondicional}, o proposición ``si y solo si'' se denota como $p \iff q$. Representa la idea de que $p$ es verdadero si y solo si $q$ también lo es. Puede verse, si es verdadera, como cumplimiento en ambos sentidos las implicaciones $p \implies q$ y $q \implies b$. Su tabla de verdad es la siguiente:
...
\index{álgebra!de proposiciones}
...
\subsubsection{Negación de una implicación} \index{negación!de una implicación}
...
\vspace{1em} \index{razonamiento deductivo válido}
...
\begin{enumerate}[label=\alph*)]
	\item Ley del \textit{modus ponens}: \label{def:ponens} \index{modus!ponens}
	...
	\item Ley del \textit{modus tollens}: \index{modus!tollens}
	\[ (p \implies q), \neg q \ \vdash \ \neg p \]
	...
	\item Ley del \textit{silogismo hipotético}: \index{silogismo!hipotético}
	...
	\item Ley del \textit{silogismo disyuntivo} (modus tollendo ponens)
	...
	\item Ley de \textit{adición} o adición disyuntiva: \index{adición!disyuntiva}
	\[ p \vdash p \lor q \]
	Notación clásica:
	\[
	\begin{array}{l}
	...
	\item Ley de \textit{adjunción} o adición conjuntiva: \index{adjunción} \index{adición!conjuntiva}
	\[ p, q \ \vdash \ p \lor q \]
	Notación clásica:
	\[
	\begin{array}{l}
...
\begin{enumerate}
	\item \textbf{Afirmación del consecuente} \index{falacia!de afirmación del consecuente}
	...
	\item \textbf{Negación del antecedente} \index{falacia!de negación del antecedente}
	\begin{itemize}
		\item Estructura:
		\[ \begin{array}{l}
			p \implies q\\
	...
	\item \textbf{Falacia del término medio no distribuido} \index{falacia!del término medio no distribuido}
	\begin{itemize}
		\item Estructura:
		\begin{itemize}[itemsep=-5pt]
	...
	\item \textbf{Falacia de la división} \index{falacia!de la división}
	\begin{itemize}
		\item Estructura:
		\begin{itemize}[itemsep=-5pt]
...
\index{negación!de cuantificadores}
\begin{enumerate}[label=\roman*)]
	...
	\begin{fmd-example}[Negación del universal] \index{negación!del universasl}
		\begin{itemize}
	...
	\begin{fmd-example}[Negación del existencial] \index{negación!del existencial}
		\begin{itemize}
...
\subsection{Axiomas de Peano} \label{sec:peano} \index{axiomas!de Peano}
...
\subsection{Principio de inducción} \label{sec:induccion} \index{principio!de inducción} \index{inducción}
...
\begin{fmd-theorem}[Principio del buen orden] \index{principio!del buen orden} \index{buen orden}
...
\subsection{Conjuntos finitos e infinitos} \label{sec:finitos} \index{conjuntos!finitos e infinitos}
\vspace{1em}
...
\begin{fmd-theorem}[Teorema de Cantor] \index{Cantor} \index{teorema!de Cantor}
