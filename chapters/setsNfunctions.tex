...
Este capítulo introduce los conceptos fundamentales de conjuntos y funciones, herramientas matemáticas esenciales en diversas áreas de la ingeniería. Se aborda la definición de conjunto, sus diferentes representaciones (extensión y comprensión), y las operaciones básicas entre conjuntos como unión, intersección, diferencia y complemento. Se estudian las relaciones de pertenencia e inclusión entre conjuntos, y el concepto de conjunto vacío y conjunto universal.
...
\begin{itemize}
	\item El \gls{conjuntouniversal}, denotado como \glsentrysymbol{conjuntouniversal}, es el que contiene todos los elementos posibles que estamos considerando en un contexto particular. Es el conjunto más grande en ese contexto y actúa como un marco de referencia para otros conjuntos más pequeños.
	...
	\item El \gls{conjuntovacio}, denotado como \glsentrysymbol{conjuntovacio}, es aquel que no contiene ningún elemento. El conjunto vacío es único, en otras palabras, todos los conjuntos vacíos son iguales.
...
Para indicar la \gls{pertenencia} de un elemento a un conjunto será utilizado el símbolo \glsentrysymbol{pertenencia}.
...
Un objeto $a$ del universal pertenece al conjunto $A$ \textbf{si y sólo si} verifica la propiedad $P(x)$. En símbolos:
\[ a \in A \iff P(a) \]
en consecuencia:
\[ a \not \in A \iff \neg P(a) \]
...
\begin{itemize}
	...
	\begin{figure}[H]
		...
		\caption*{\(A^c\)}
	\end{figure}
	\item  Ejemplo: Si \(U\) es el conjunto de números naturales y \(A\) el conjunto de números pares,  \(A = \{2, 4, 6, \ldots \}\), entonces \(A^c\) es el conjunto de números impares, \(A^c = \{1, 3, 5, 7, \ldots\}\).
	\item Propiedades:
	\begin{enumerate}
...
\begin{fmd-definition}[Conjunto cociente]
...
Una \textbf{función} de $A$ en $B$ asigna a cada elemento de $A$ un único elemento de $B$.
...
\begin{fmd-example}[Función] \label{ex:funcion}
...
\begin{fmd-definition}[Función]
	$f$ es una función o aplicación de $A$ en $B$ si y sólo si $f$ es una relación
	entre $A$ y $B$, tal que todo elemento de $A$ tiene un único correspondiente en
	$B$.
\end{fmd-definition}

o bien:
\begin{definition}[Función]
	$f$ es una función o aplicación de $A$ en $B$ si y sólo si $f$ es un subconjunto
	de $A \times B$ que satisface las siguientes condiciones de existencia y unicidad:
	\begin{enumerate}
		\item $\forall a \in A, \ \exists \ b \in B / (a, b) \in f$
		\item $(a, b) \in f \wedge (a, c) \in f \implies b = c$
	\end{enumerate}
\end{definition}
...
\begin{lgnote}
	Si una relación es función, la relación inversa no lo es necesariamente
\end{lgnote}
...
\begin{example}
	...
	\begin{enumerate}
		...
		\begin{table}
			...
			\end{tabular}
		\end{table}
		El elemento 1 de $B$ carece de antecedente o \textit{preimagen}\footnote{Ver definición \ref{def:preimagen}, pág. \pageref{def:preimagen}.}  en $A$.
	\end{enumerate}
...
\begin{itemize}
	...
	en el codominio, entonces $f$ se llama función \textit{inyectiva} o uno a uno.
	\item Por otra parte, si todo elemento del codominio es imagen de algún elemento 
	del dominio, la función se llama \textit{sobreyectiva}.
	\item Cuando se presentan ambas situaciones simultáneamente, la función se llama 
	\textit{biyectiva} o correspondencia biunívoca.
\end{itemize}
...
\subsubsection{Función sobreyectiva}
\vspace{1em}
...
\end{fmd-definition}
\begin{itemize}
	\item El conjunto de las imágenes (\textit{rango} o \textit{recorrido}) se identifica con el codominio de la función. No hay elementos del codominio que no sea la imagen de \textit{al menos un} elemento del dominio, en otras palabras, el rango y el codominio coinciden.
	\item El ejemplo 3 corresponde a funciones sobreyectivas.
...
\end{fmd-definition}
\begin{itemize}
	\item Las funciones del ejemplo 3 son biyectivas.
\end{itemize}
...
\begin{figure}[H]
	...
	\begin{minipage}{.32\textwidth}
		...
		\begin{tikzpicture}[scale=.7]
			...
			% Caption
			\node at (1.5,-2.7) {No es biyectiva};
		\end{tikzpicture}
	\end{minipage}
	\begin{minipage}{.32\textwidth}
		\centering
...
\begin{itemize}
	...
	La función $i_A$ es obviamente \textbf{biyectiva}.
		\begin{figure}[H]
			\centering
			\begin{tikzpicture}
...
\begin{itemize}
	...
	Tales funciones se llaman \textit{primera y segunda proyección del producto
		cartesiano}, y asignan a cada par ordenado la primera y la segunda componentes,
	respectivamente. En un gráfico cartesiano se tiene:
...
\begin{fmd-definition}[Composición de funciones]
	Composición de las funciones $f: A \rightarrow B$ y $g:B \rightarrow C$ es la
...
\begin{definition}[Funciones iguales]
...
\subsection{Funciones inversas} \label{sec:finversas}
...
\begin{fmd-example}
	...
	La función $f$ no es necesariamente sobreyectiva. Nos apoyamos en el diagrama de la fig: \ref{fig:inversas2}.
	\begin{figure}[H]
		\centering
		\begin{tikzpicture}[scale=1]
			% Coordenadas de puntos
...
\begin{enumerate}[label=\textbf{\alph*)}]
	...
	\[ f: X \rightarrow Y, A \subset B, B \subset X \mbox{ y } A \subset B \implies
	f(A) \subset f(B)\]
	En efecto, sea
	\[ z \in f(A) \rightarrow \exists \, x \in A / f(x) = z \rightarrow \exists \, x
...
\begin{enumerate}[label=\textbf{\alph*)}]
	...
	\[ f: X \rightarrow Y, A \subset X \land B \subset X \implies f\left( 
	...
	Resulta
	\[ f\left( A \cap B \right) \subsetneqq f(A) \cap f(B) \]
\end{enumerate}
...
\begin{fmd-definition}[Preimagen] \label{def:preimagen}
...
\begin{enumerate}[label=\textbf{\alph*)}]
	...
	f(x) \in A \vee f(x) \in B \]
	\[ \rightarrow x \in f^{-1}(A) \vee x \in f^{-1}(B) \rightarrow x \in f^{-1}
	(A) \cup f^{-1} (B)\]
\end{enumerate}
...
\begin{enumerate}[label=\textbf{\alph*)}]
	...
	Se tiene
	\[ x \in f^{-1}\left( A \cap B \right) \iff f(x) \in A \cap B \iff f(x) \in A
	\land f(x) \in B \]
	\[ \iff x \in f^{-1}(A) \land x \in f^{-1}(B) \iff x \in f^{-1}(A) \cap f^{-1}
	(B) \]
\end{enumerate}
...
\begin{enumerate}[label=\textbf{\alph*)}]
	...
	En efecto:
	\[ x \in f^{-1}\left( A^c \right) \iff f(x) \in A^c \iff f(x) \not \in A 
	\iff \sim \left[ f(x) \in A \right] \]
	\[ \iff \sim \left[ x \in f^{-1}(A) \right] \iff x \not \in f^{-1}(A) \iff 
	x \in \left[ f^{-1}(A) \right]^c\]
\end{enumerate}
...
\begin{fmd-definition}[Extensión de una función]
