...
\begin{fmd-definition}[Complemento]
	El complemento de un conjunto \(A\) con respecto a un conjunto universal \(U\) contiene todos los elementos que no están en \(A\).
...
\begin{fmd-example} \label{ex:funcion}
...
\emph{Si una relación es función, la relación inversa no lo es necesariamente}.
...
\begin{fmd-definition}[Función inyectiva]
	$f:A \rightarrow B \mbox{ es inyectiva} \iff \forall x', x'' \in A:
	x' \ne x'' \implies f(x') \ne f(x'')$
\end{fmd-definition}\vspace{1mm}

Mediante la contrarrecíproca:
\[ f:A \rightarrow B \mbox{ es inyectiva} \iff \forall x', x'' \in A: 
f(x') = f(x'') \implies x' = x'' \]
...
\begin{fmd-definition}[Función sobreyectiva]
	$f:A \rightarrow B \mbox{ es sobreyectiva} \iff \forall y \in B, \exists \ x \in A
	/ y = f(x)$
\end{fmd-definition}
\begin{itemize}
	\item El conjunto de las imágenes (\textit{rango} o \textit{recorrido}) se identifica con el codominio de la función. Es decir, no hay elementos del codominio que no sea la imagen de \textit{al menos un} elemento del dominio, en otras palabras, el rango y el codominio coinciden.
...
\begin{fmd-definition}[Función biyectiva]
	$f:A \rightarrow B \mbox{ es biyectiva} \iff f \mbox{ es inyectiva y sobreyectiva}$.
\end{fmd-definition}
\begin{itemize}
	\item Las funciones del ejemplo 3 son biyectivas.
\end{itemize}
Negando el antecedente y el consecuente de la doble implicación, se tiene:
\[ f: A \rightarrow B \mbox{ no es biyectiva } \iff f \mbox{ no es inyectiva o 
	no es sobreyectiva.} \]
...
\begin{fmd-example}
	...
	Definimos $f: P(A) \rightarrow P(B)$ mediante $f(X) = X \cap B$, es decir, la imagen
...
\begin{itemize}
	...
	Es decir, la identidad en $A$ es la diagonal de $A^2$. Como relación es reflexiva,
...
primera sea parte del dominio de la segunda, es decir: $B \subset B'$. En la fig. \ref{fig:compos1} se ve gráficamente esta afirmación.
...
\begin{fmd-example} \label{ex:compos}
	Sean: \[ \begin{cases}
...
\begin{enumerate}[label=\textbf{\alph*)}]
	...
	vale para sus imágenes. Es decir:
...
\begin{enumerate}[label=\textbf{\alph*)}]
	...
	unión de sus imágenes. Es decir:
...
\begin{fmd-definition}[Restricción de una función]
	Sean $f:X \rightarrow Y$, $A \subset X$ y la función $g: A \rightarrow Y / 
