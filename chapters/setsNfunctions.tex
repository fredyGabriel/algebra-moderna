...
\begin{itemize}
	...
	\begin{enumerate}[label=\roman*)]
		\item \textit{Reflexibidad}: Todo conjunto es parte de sí mismo.
		\[ A \subset A \]
		\item \textit{Transitividad}: Si un conjunto es parte de otro y este es parte de un tercero, el primero está incluido en el tercero:
		\[\mbox{Si } A \subset B \mbox{ y } B \subset C \mbox{ entonces } A \subset C \]
		\item \textit{Antisimetría}: Si un conjunto es parte de otro y éste es parte del primero, entonces son iguales\footnote{Este hecho se usa para establecer la igualdad de conjuntos dada en la definición \ref{def:igualdad_conjuntos}}.
...
\begin{itemize}
	...
	\item \textbf{Observaciones}
	\begin{enumerate}[label=\roman*)]
		\item No es conmutativa: \( A \setminus B \neq B \setminus A \)
		\item No es asociativa: \( \left(A \setminus B\right) \setminus C \neq A \setminus \left( B \setminus C\right) \)
	\end{enumerate}
	\item \textbf{Propiedades}:
	\begin{enumerate}[label=\roman*)]
		\item Elemento neutro por la derecha: \( A \setminus \emptyset = A \)
		\item Elemento absorbente por la izquierda: \( \emptyset \setminus A = \emptyset \) 
		...
		\item Idempotencia: \( A \setminus A = \emptyset \)
		\item Diferencia con el conjunto universal:
		\begin{enumerate}
			\item \( U \setminus A = A^c \)
			\item \( A \setminus U = \emptyset \)
		\end{enumerate}
		\item Diferencia con el conjunto vacío:
		\begin{enumerate}
			\item \( A \setminus \emptyset = A \)
			\item \( \emptyset \setminus A = \emptyset \)
		\end{enumerate}
		\item Distributividad por la izquierda respecto de la intersección: \[ A \cap (B \setminus C) = (A \cap B) \setminus (A \cap C) \]
		\item Distributividad por la derecha respecto de la unión: \[ (A \cup B) \setminus C = (A \setminus C) \cup (B \setminus C) \]
		\item Otras propiedades:
		\begin{enumerate}
			\item \( A \subset B \iff A \setminus B = \emptyset \)
			\item $A \setminus (A \setminus B) = A \cap B$
		\end{enumerate}
	\end{enumerate}
\end{itemize}
\begin{lgnote}
	El símbolo $\iff$ es la doble implicación o bicondicional ``\textit{si y solo si}''. Se explica con rigor en \ref{sec:bicondicional}.
\end{lgnote}

\begin{example}[Diferencia de conjuntos]
	Si \(A = \{1, 2, 3\}\) y \(B = \{3, 4, 5\}\), entonces \(A \setminus B = \{1, 2\}\).
\end{example}
...
\begin{fmd-definition}[Producto cartesiano] \label{def:producto_cartesiano}
...
\begin{itemize}
	...
	\item \textbf{Propiedades}
	\begin{enumerate}[label=\roman*)]
		\item Distributividad respecto de la unión
		\[ \left( A \cup B \right) \times C = \left( A \times C \right) \cup \left( B \times C \right) \]
		\[ A \times (B \cup C) = (A \times B) \cup (A \times C) \]
		\item Distributividad respecto de la intersección
		\[ \left( A \cap B \right) \times C = \left( A \times C \right) \cap \left( B \times C \right) \]
	\[ A \times (B \cap C) = (A \times B) \cap (A \times C) \]
		\item Distributividad respecto de la diferencia
	\[ \left( A \setminus B \right) \times C = \left( A \times C \right) \setminus \left( B \times C \right) \]
	\[ A \times (B \setminus C) = (A \times B) \setminus (A \times C) \]
	\end{enumerate}
	
	\end{itemize}
	\begin{fmd-definition}[Complemento del producto cartesiano]
	
	De la definición \ref{def:producto_cartesiano}, el complemento de un producto cartesiano se define como el conjunto de todos los pares ordenados que pertenecen al producto cartesiano del universo $U \times U$, pero que no pertenecen a $A \times B$.
	\[ \left(A \times B\right)^c = \left(U \times U\right) \setminus \left( A \times B \right) \]
	\end{fmd-definition}
	\begin{example}[Complemento de un producto cartesiano]
		
		Sean \( U = \{ 1, 2, 3 \} \), \( A = \{ 1, 2 \} \), \( B = \{2, 3\} \). Entonces:
		\begin{itemize}
			\item $U \times U = \{ (1, 1), (1,2), (1,3), (2, 1), (2,2), (2, 3), (3,1), (3,2), (3,3) \}$
			\item $A \times B = \{ (1, 2), (1,3), (2,2), (2, 3) \}$
			\item $\left(A \times B\right)^c = \{ (1, 1), (2,1), (3,1), (3, 2), (3,3) \}$
		\end{itemize}
	\end{example}
	
	Alternativamente:
	\[(A \times B)^c = \{ (x, y) \mid x \in A^c \text{ o } y \in B^c \} = \{ (x, y) \mid x \not \in A \text{ o } y \not \in B \}\]
	de donde:
	\[ (A \times B)^c = (A^c \times B^c) \cup (A^c \times B) \cup (A \times B^c) \]
	y, por lo tanto:
	\begin{enumerate}
		\item \( (A \times B)^c \neq A^c \times B^c \)
		\item \(A^c \times B^c  \subset (A \times B)^c\)
	\end{enumerate}
	
\begin{lgnote}
	Es fundamental definir el conjunto universal $U$ para poder determinar el complemento de un producto cartesiano.
\end{lgnote}
...
\begin{enumerate}
	...
	\item Existencia de inversas\footnote{ibid.}:
	\begin{itemize}
...
\begin{enumerate}
	...
	\item Sea $U$ un conjunto finito con $|U| = n$. Sean $A$ y $B$ subconjuntos de $U$.
	\begin{enumerate}
		\item Exprese la cardinalidad de $(A \times B)^c$ en términos de $n$, $|A|$ y $|B|$.
		\item Si $|A| = |B| = n/2$, ?`cuál es la cardinalidad máxima posible de $(A \times B)^c$? Justifique su respuesta.
	\end{enumerate}
	...
	\item \textit{Paradoja de Russell}. Llamemos $R$ al conjunto de todos los conjuntos que no se contienen a sí mismos. Es decir: \( R = \{ x \mid x \not \in x \} \), en donde $x$ representa a tales conjuntos. ?`Pertenece $R$ a sí mismo? Analiza y discute con tus compañeros ambas posibilidades ($R \in R$ y $R \not \in R$).
